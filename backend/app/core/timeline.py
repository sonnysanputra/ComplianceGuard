...
Folded into the Transaction Analysis Agent (it used to be a separate node) so the
investigation stays a single focused step instead of two agents over the same data.
...
def build_timeline(txns: list) -> tuple[dict, list]:
    """Return (timeline_findings, evidence_items) for a customer's transactions."""
    ...
    for t in ordered:
        ...
        if near:
            near_count += 1
        note = _risk_note(direction, amount, country, recipient, is_new,
